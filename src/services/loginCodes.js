...
function expiryStamp(value) {
  if (value instanceof Date) return sqlTime(value);
...
async function findParticipant(identity) {
  if (!identity || identity.audience !== 'participant') return null;

  if (identity.type === 'email') {
    return (await db.prepare('SELECT * FROM users WHERE lower(email) = ?').get(identity.value)) || null;
  }
  ...
  const matches = await db.prepare('SELECT * FROM users WHERE phone_normalized = ?').all(identity.value);
  return matches.length === 1 ? matches[0] : null;
}

// ─── Issuing ────────────────────────────────────────────────

async function recentRequests(identifier) {
  const row = await db.prepare(`
    SELECT COUNT(*) as c FROM login_codes
    WHERE identifier = ? AND created_at > datetime('now', ?)
  `).get(identifier, `-${WINDOW_SEC} seconds`);
  return Number(row?.c || 0);
}

async function throttled(identifier) {
  return (await recentRequests(identifier)) >= MAX_PER_WINDOW;
}
...
async function issue(user, identity) {
  const code = generateCode();

  // Sequential statements rather than db.transaction: the Postgres adapter's
  // transaction helper is async and does not await inner .run() calls.
  await db.prepare(`
...
async function verify(identity, submitted) {
  ...
  if (!row || expiryStamp(row.expires_at) <= sqlTime(new Date())) {
  ...
  if (!matches) {
    ...
    if (attempts >= MAX_ATTEMPTS) {
      await db.prepare("UPDATE login_codes SET attempts = ?, consumed_at = datetime('now') WHERE id = ?")
        .run(attempts, row.id);
      return { ok: false, error: 'Too many wrong attempts. Ask for a new code.' };
    }
    await db.prepare('UPDATE login_codes SET attempts = ? WHERE id = ?').run(attempts, row.id);
    return { ok: false, error: 'That code is not valid. Check it and try again.' };
  }

  await db.prepare("UPDATE login_codes SET consumed_at = datetime('now') WHERE id = ?").run(row.id);
  return { ok: true, user };
}
